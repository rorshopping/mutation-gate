...
def render_report(report: Report) -> str:
    ...
    if report.survived:
        ...
        for r in report.surviving():
            m = r.mutant
            lines.append(
                f"  {m.file.as_posix()}:{m.lineno}  [{m.operator}]  {m.before}  →  {m.after}  "
                f"({r.duration:.2f}s)"
            )
    else:
...
def render_verify(vr: VerifyResult, gate_score: float | None = None) -> str:
    ...
    if vr.survivors:
        ...
        for m in vr.survivors:
            lines.append(f"  {m.file.as_posix()}:{m.lineno}  [{m.operator}]  {m.before}  →  {m.after}")
    return "\n".join(lines)
...
def html_report(report: Report, min_score: float | None = None) -> str:
    """Render a self-contained HTML report (no external assets)."""
    score = report.score
    score_color = "#16a34a" if score is not None and score >= 0.8 else ("#ca8a04" if score is not None and score >= 0.6 else "#dc2626")
    score_html = f"{score * 100:.1f}%" if score is not None else "n/a"
    if report.baseline_failed:
        return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Mutation Gate</title></head>
<body style="font-family:-apple-system,system-ui,sans-serif;max-width:900px;margin:40px auto;padding:0 20px;color:#111">
<h1>🧬 Mutation Gate</h1><p style="color:#dc2626;font-weight:600">Baseline test run failed — cannot run mutation gate.</p>
<pre style="background:#f6f8fa;padding:16px;border-radius:8px;overflow:auto">{_esc(report.baseline_output[:2000])}</pre>
</body></html>"""
    ...
    for r in report.surviving():
        m = r.mutant
        rows.append(
            f"<tr><td><code>{_esc(m.file)}</code></td><td>{m.lineno}</td>"
            f"<td><code>{_esc(m.operator)}</code></td>"
            f"<td><code>{_esc(m.before)}</code> → <code>{_esc(m.after)}</code></td>"
            f"<td>{r.duration:.2f}s</td></tr>"
        )
    survivors_html = "\n".join(rows) if rows else '<tr><td colspan="5" style="text-align:center;padding:16px">All mutants killed — tests have real teeth. 🎉</td></tr>'

    gate_html = ""
    if min_score is not None and score is not None:
        ok = score >= min_score
        gate_html = (
            f'<p style="font-size:18px">Gate (min {min_score * 100:.0f}%): '
            f'<span style="color:{"#16a34a" if ok else "#dc2626"};font-weight:700">{"PASS ✅" if ok else "FAIL ❌"}</span></p>'
        )

    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Mutation Gate — {score_html}</title></head>
<body style="font-family:-apple-system,system-ui,sans-serif;max-width:900px;margin:40px auto;padding:0 20px;color:#111">
<h1>🧬 Mutation Gate</h1>
<p style="font-size:24px;font-weight:700;color:{score_color}">{score_html}
<span style="font-size:14px;color:#555;font-weight:400">({report.killed}/{report.total_counted} mutants killed</span></span>
{"" if not report.cached else f'<span style="font-size:14px;color:#555"> · {report.cached} from cache</span>'}
</p>
{gate_html}
<h2 style="margin-top:32px">Surviving mutants <span style="color:#888;font-size:14px;font-weight:400">({report.survived})</span></h2>
<table style="border-collapse:collapse;width:100%;font-size:14px">
<thead><tr style="text-align:left;border-bottom:2px solid #ddd">
<th style="padding:8px">File</th><th style="padding:8px">Line</th><th style="padding:8px">Operator</th>
<th style="padding:8px">Before → After</th><th style="padding:8px">Time</th></tr></thead>
<tbody>{survivors_html}</tbody></table>
<p style="color:#888;font-size:12px;margin-top:32px">Generated by mutation-gate</p>
</body></html>"""
